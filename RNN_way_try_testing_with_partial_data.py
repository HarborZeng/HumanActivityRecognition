...
def segment_signal(data, window_size=90):
    ...
    for (start, end) in windows(data['Time_Stamp'], window_size):
        ...
        if len(data['Time_Stamp'][int(start):int(end)]) == window_size:
...
dataset = read_data('shoab_data/Combined_data.xlsx')
...
dataset_test = read_data('shoab_data/Wrist.xlsx')
dataset_test['Ax'] = feature_normalize(dataset_test['Ax'])
dataset_test['Ay'] = feature_normalize(dataset_test['Ay'])
dataset_test['Az'] = feature_normalize(dataset_test['Az'])
dataset_test['Gx'] = feature_normalize(dataset_test['Gx'])
dataset_test['Gy'] = feature_normalize(dataset_test['Gy'])
dataset_test['Gz'] = feature_normalize(dataset_test['Gz'])
dataset_test['Mx'] = feature_normalize(dataset_test['Mx'])
dataset_test['My'] = feature_normalize(dataset_test['My'])
dataset_test['Mz'] = feature_normalize(dataset_test['Mz'])
segments_test, labels_test = segment_signal(dataset_test)
labels_test = np.asarray(pd.get_dummies(labels_test), dtype=np.int8)
reshaped_segments_test = segments_test.reshape(len(segments_test), 1, 90, 9)
